...
-type 'PingPongRequest'() ::
      #{number_of_messages      => non_neg_integer() % = 1, 64 bits
       }.

-type 'ThroughputPingPongRequest'() ::
      #{messages_per_pair       => non_neg_integer(), % = 1, 64 bits
        pipeline_size           => non_neg_integer(), % = 2, 64 bits
...
-type 'AtomicRegisterRequest'() ::
      #{read_workload           => float() | integer() | infinity | '-infinity' | nan, % = 1
        ...
        number_of_keys          => non_neg_integer() % = 4, 64 bits
       }.

-type 'SizedThroughputRequest'() ::
      #{message_size            => non_neg_integer(), % = 1, 32 bits
        batch_size              => non_neg_integer(), % = 2, 32 bits
        number_of_batches       => non_neg_integer(), % = 3, 32 bits
        number_of_pairs         => non_neg_integer() % = 4, 32 bits
       }.
...
-spec encode_msg('PingPongRequest'() | 'ThroughputPingPongRequest'() | 'AtomicRegisterRequest'() | 'SizedThroughputRequest'() | 'TestResult'() | 'TestSuccess'() | 'TestFailure'() | 'NotImplemented'() | 'ReadyRequest'() | 'ReadyResponse'() | 'ShutdownRequest'() | 'ShutdownAck'(), atom()) -> binary().
...
encode_msg_SizedThroughputRequest(#{} = M, Bin,
                                  TrUserData) ->
    B1 = case M of
             #{message_size := F1} ->
                 begin
...
dfp_read_field_def_SizedThroughputRequest(<<>>, 0, 0,
                                          F@_1, F@_2, F@_3, F@_4, _) ->
    #{message_size => F@_1, batch_size => F@_2,
      number_of_batches => F@_3, number_of_pairs => F@_4};
...
merge_msg_SizedThroughputRequest(PMsg, NMsg, _) ->
    S1 = #{},
    S2 = case {PMsg, NMsg} of
             {_, #{message_size := NFmessage_size}} ->
                 S1#{message_size => NFmessage_size};
             {#{message_size := PFmessage_size}, _} ->
                 S1#{message_size => PFmessage_size};
             _ -> S1
         end,
    S3 = case {PMsg, NMsg} of
    ...
    S4 = case {PMsg, NMsg} of
             {_, #{number_of_batches := NFnumber_of_batches}} ->
                 S3#{number_of_batches => NFnumber_of_batches};
             {#{number_of_batches := PFnumber_of_batches}, _} ->
                 S3#{number_of_batches => PFnumber_of_batches};
             _ -> S3
         end,
    case {PMsg, NMsg} of
        {_, #{number_of_pairs := NFnumber_of_pairs}} ->
            S4#{number_of_pairs => NFnumber_of_pairs};
        {#{number_of_pairs := PFnumber_of_pairs}, _} ->
            S4#{number_of_pairs => PFnumber_of_pairs};
        _ -> S4
    end.
...
v_msg_SizedThroughputRequest(#{} = M, Path,
                             TrUserData) ->
    case M of
        #{message_size := F1} ->
    ...
    lists:foreach(fun (message_size) -> ok;
                  ...
                  maps:keys(M)),
    ok;
v_msg_SizedThroughputRequest(M, Path, _TrUserData)
    when is_map(M) ->
    mk_type_error({missing_fields,
                   [] -- maps:keys(M),
                   'SizedThroughputRequest'},
...
get_msg_defs() ->
    [{{msg, 'PingPongRequest'},
     ...
     {{msg, 'SizedThroughputRequest'},
      [#{name => message_size, fnum => 1, rnum => 2,
         type => uint32, occurrence => optional, opts => []},
       #{name => batch_size, fnum => 2, rnum => 3,
         type => uint32, occurrence => optional, opts => []},
       #{name => number_of_batches, fnum => 3, rnum => 4,
         type => uint32, occurrence => optional, opts => []},
       #{name => number_of_pairs, fnum => 4, rnum => 5,
         type => uint32, occurrence => optional, opts => []}]},
     {{msg, 'TestResult'},
